...
import ParserCombinators (Parser, char, doParse, filter, satisfy, string, sepBy, between)
import System.IO
...
emptyContainer =
  Element <$> between (char '<') text (string "/>") <*> pure []
...
container p =
  liftA3 (\ot d ct -> Element ot d)
         (between (char '<') text (char '>'))
         (many p)
         (between (string "</") text (char '>'))
...
Just
(Element "body"
   [Element "h1" [PCDATA "A Midsummer Night's Dream"]
   ,Element "h2" [PCDATA "Dramatis Personae"]
   ,PCDATA "THESEUS, Duke of Athens."
   ,Element "br" []
   ,PCDATA "EGEUS, father to Hermia."
   ,Element "br" []],"")
~~~~~
